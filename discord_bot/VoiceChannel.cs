...
namespace discord_bot
{
	...
	{
		private const string NAME = "セックスしないと出られない部屋";
		protected SexRoom() { }
		...
		public static async Task<SexRoom> Construct(ISocketMessageChannel text_channel, SocketGuild guild)
		{
			...
			if (speaking_users.Count <= 1)
			{
				await text_channel.SendMessageAsync($"{ ErrorMessage[Error.NotEnoughUsers] }");
				return null;
			}
			var targets = speaking_users.Count == 1
				? ChooseRandom(speaking_users)
				: ChooseRandom(speaking_users, 2);
			var voice_channel = new SexRoom(await VoiceChannel.Construct(guild, prop => {prop.CategoryId = targets[0].VoiceChannel.CategoryId;prop.UserLimit = 2; }, NAME)) ;
			await voice_channel.Attract(targets);
			/*if (targets.Count == 1)
			{	
				await (voice_channel as IAudioChannel).ConnectAsync();
			}*/
			Program.client.UserVoiceStateUpdated += voice_channel.DeleteIfEmpty;
